...
| TypeScript, renderer, Rust, frontend, and audit gates | PASS WITH PTY TEST-RACE FOLLOW-UP | TypeScript, 19 renderer tests, release metadata, the 320-module frontend build, Cargo check, and full npm audit pass. The real PTY test first timed out twice, then a no-product-code probe isolated an early-CPR/late-DSR race plus a missing `cmd` CPR response. Six warmed `pwsh` runs and the final serial Rust suite passed (23 passed, one network gate ignored). The product xterm path responds to actual output, but the test still needs deterministic DSR synchronization before release. |
